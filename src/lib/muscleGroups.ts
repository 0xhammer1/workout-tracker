...
const INFO: Record<MuscleGroup, MuscleInfo> = {
  ...
  quads:      { group: 'quads',      label: 'Quads',      color: '#86efac', bg: 'rgba(34, 197, 94, 0.15)' },
  hamstrings: { group: 'hamstrings', label: 'Hamstrings', color: '#5eead4', bg: 'rgba(20, 184, 166, 0.15)' },
  legs:       { group: 'legs',       label: 'Legs',       color: '#a3e635', bg: 'rgba(132, 204, 22, 0.15)' },
...
export const ALL_MUSCLE_GROUPS: MuscleGroup[] = [
  'chest', 'back', 'shoulders', 'biceps', 'triceps', 'quads', 'hamstrings', 'legs', 'glutes', 'core', 'other',
]
...
export function muscleFor(exerciseName: string): MuscleInfo {
  ...
  if (/(leg.?curl|hamstring.?curl|nordic|glute.?ham|rdl|romanian|stiff.?leg|good.?morning|hamstring)/i.test(n)) return INFO.hamstrings
  if (/(hip.?thrust|glute.?bridge|glute.?kickback|glute)/i.test(n)) return INFO.glutes
  if (/(squat|leg.?press|leg.?extension|lunge|step.?up|hack|sissy|bulgarian|quad)/i.test(n)) return INFO.quads
  if (/(calf|calves)/i.test(n)) return INFO.legs
  if (/(deadlift|row|pulldown|pull.?up|chin.?up|lat\b|back)/i.test(n)) return INFO.back
...
export function majorGroupFor(ex: { name: string; muscle_group?: string | null }): MajorGroup | null {
  ...
  if (m === 'chest' || m === 'back' || m === 'shoulders' || m === 'biceps' || m === 'triceps') return m
  if (m === 'quads' || m === 'hamstrings') return m
  return null
}
